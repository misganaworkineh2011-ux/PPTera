...
export default function TopBar({ credits = 0, onSearch }: TopBarProps) {
  ...
  return (
    <header className="md:sticky top-0 z-30 flex h-14 lg:h-20 items-center justify-between bg-[#F8F9FA] dark:bg-black px-3 sm:px-4 lg:px-8 border-b border-slate-100/50 dark:border-neutral-800 backdrop-blur-sm gap-2 lg:gap-4">
      {/* Left: Mobile menu button + Title */}
      <div className="flex items-center gap-2 lg:gap-3 shrink-0">
        {/* Mobile menu button */}
        <button
          onClick={() => setIsMobileSidebarOpen(true)}
          className="lg:hidden p-2 rounded-lg text-slate-600 hover:text-slate-900 hover:bg-slate-100 dark:text-neutral-400 dark:hover:text-white dark:hover:bg-neutral-900"
        >
          <Menu size={22} />
        </button>
      ...
      <div className="flex items-center gap-2 lg:gap-3 flex-1 justify-end">
        ...
        <div className={`relative transition-all hidden sm:block ${isTitleSticky ? "max-w-xs" : "max-w-md"} ${isTitleSticky ? "w-48 lg:w-64" : "w-full"}`}>
          <Search className="absolute left-3 lg:left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400 dark:text-neutral-500" />
          <input
            type="text"
            value={searchQuery}
            onChange={handleSearchChange}
            placeholder={t.search || "Search..."}
            className="w-full rounded-full border-none bg-white dark:bg-neutral-900 pl-9 lg:pl-11 pr-3 lg:pr-4 py-2 lg:py-2.5 text-sm font-medium text-slate-700 dark:text-white shadow-sm ring-1 ring-slate-200 dark:ring-neutral-800 placeholder:text-slate-400 dark:placeholder:text-neutral-500 focus:outline-none focus:ring-2 focus:ring-[#1e3a8a]/20 dark:focus:ring-white/20"
          />
        </div>
        ...
        <button
          ...
          className="sm:hidden p-2 rounded-full bg-white dark:bg-neutral-900 text-slate-500 dark:text-neutral-400 shadow-sm ring-1 ring-slate-200 dark:ring-neutral-800 hover:bg-slate-50 dark:hover:bg-neutral-800"
        >
          <Search size={18} />
        </button>
        ...
        <div className="relative notifications-container">
          <button
            ...
            className="relative flex h-9 w-9 lg:h-10 lg:w-10 items-center justify-center rounded-full bg-white dark:bg-neutral-900 text-slate-500 dark:text-neutral-400 shadow-sm ring-1 ring-slate-200 dark:ring-neutral-800 transition hover:bg-slate-50 dark:hover:bg-neutral-800 hover:text-slate-700 dark:hover:text-white"
          >
            <Bell size={18} className="lg:hidden" />
            <Bell size={20} className="hidden lg:block" />
            {unreadCount > 0 && (
              <span className="absolute right-2 top-2 lg:right-2.5 lg:top-2.5 h-2 w-2 rounded-full border-2 border-white dark:border-neutral-900 bg-[#06b6d4]"></span>
            )}
          </button>

          {/* Notifications Dropdown */}
          {showNotifications && (
            <div className="absolute right-0 top-full mt-2 w-[calc(100vw-2rem)] sm:w-80 max-w-sm rounded-lg border border-slate-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow-2xl z-50">
              ...
              <div className="max-h-96 overflow-y-auto">
                {loading ? (
                  ...
                  <div className="p-8 text-center">
                    <Bell size={32} className="mx-auto mb-2 text-slate-300 dark:text-neutral-700" />
                    <p className="text-sm text-slate-500 dark:text-neutral-500">{t.noNotifications || "No notifications yet"}</p>
                  </div>
                ) : (
                  notifications.map((notif) => (
                    <div
                      ...
                      className={`border-b border-slate-50 dark:border-neutral-800 p-4 transition hover:bg-slate-50 dark:hover:bg-neutral-800 cursor-pointer ${
                    ...
                    >
                      <div className="flex gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-400 dark:text-neutral-600 mt-1">{getTimeAgo(notif.createdAt)}</p>
                        </div>
                      </div>
                    </div>
                  ))
      ...
      {showMobileSearch && (
        <div className="absolute left-0 right-0 top-full bg-white dark:bg-black border-b border-slate-200 dark:border-neutral-800 p-3 sm:hidden shadow-lg">
          <div className="relative">
            ...
            <input
              ...
              placeholder={t.search || "Search..."}
              autoFocus
              className="w-full rounded-full border-none bg-slate-50 dark:bg-neutral-900 pl-10 pr-4 py-2.5 text-sm font-medium text-slate-700 dark:text-white ring-1 ring-slate-200 dark:ring-neutral-800 placeholder:text-slate-400 dark:placeholder:text-neutral-500 focus:outline-none focus:ring-2 focus:ring-[#1e3a8a]/20 dark:focus:ring-white/20"
            />
          </div>
        </div>
      )}
