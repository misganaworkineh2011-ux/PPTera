...
${currentContent}`;
    ...
    if (gemini) {
      try {
        console.log("[edit-slide] Using Gemini API...");
        ...
        console.warn("[edit-slide] Gemini failed, falling back to OpenAI:", geminiError);
        // Fallback to OpenAI
    ...
    try {
      editedSlide = parseAIJson<SlideContent>(responseText);
    } catch (parseError) {
