...
import { BookOpen, TrendingUp, Zap, Clock, ArrowRight } from "lucide-react";
...
interface InsightsPageClientProps {
  ...
  initialFeaturedPost: InsightPost | null;
  stats: { value: string; label: string }[];
  translations: {
    insights?: string;
    insightsHeroTitle?: string;
    insightsHeroHighlight?: string;
...
export function InsightsPageClient({
  ...
  const [featuredPost] = useState<InsightPost | null>(initialFeaturedPost);
  ...
  const loadMore = async () => {
    ...
    try {
      const response = await fetch(
        `/api/insights?category=${selectedCategory}&limit=9&offset=${posts.length}`
      );
      if (response.ok) {
  ...
  return (
    <>
      ...
      {featuredPost && (
        <section className="relative px-6 pb-24">
          <div className="mx-auto max-w-7xl">
            <article className="rounded-3xl overflow-hidden border border-slate-200 bg-white hover:shadow-2xl transition-all duration-300 animate-fade-in-up [animation-delay:700ms]">
