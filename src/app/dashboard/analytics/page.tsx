...
export default function AnalyticsPage() {
      ...
      <div className="max-w-7xl mx-auto space-y-8 p-4 sm:p-6 lg:p-8">
        ...
        <div className="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="bg-white dark:bg-neutral-900 rounded-2xl border border-slate-200 dark:border-neutral-800 p-4 sm:p-6">
              <div className="flex items-center justify-between mb-3 sm:mb-4">
                <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-xl bg-slate-200 dark:bg-neutral-800 animate-pulse" />
                <div className="w-10 h-4 sm:w-12 sm:h-5 rounded bg-slate-100 dark:bg-neutral-800 animate-pulse" />
              </div>
              <div className="h-6 sm:h-8 w-12 sm:w-16 bg-slate-200 dark:bg-neutral-800 rounded animate-pulse mb-2" />
              <div className="h-3 sm:h-4 w-20 sm:w-28 bg-slate-100 dark:bg-neutral-800 rounded animate-pulse" />
            </div>
          ))}
        </div>
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-slate-200 dark:border-neutral-800 p-4 sm:p-6">
          <div className="h-5 w-32 bg-slate-200 dark:bg-neutral-800 rounded animate-pulse mb-4 sm:mb-6" />
          <div className="h-40 sm:h-48 flex items-end gap-[2px] sm:gap-1">
            {Array.from({ length: 30 }).map((_, i) => (
              <div
                key={i}
                className="flex-1 min-w-[6px] sm:min-w-[8px] bg-slate-200 dark:bg-neutral-800 rounded-t animate-pulse"
            ...
            <div className="space-y-2 sm:space-y-3">
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="flex items-center gap-2 sm:gap-4 p-2 sm:p-3">
                  <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-slate-200 dark:bg-neutral-800 animate-pulse shrink-0" />
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 p-4 sm:p-6 lg:p-8">
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-slate-200 dark:border-neutral-800 p-4 sm:p-6">
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <div className="p-2 sm:p-3 bg-cyan-100 dark:bg-cyan-900/30 rounded-xl">
              <Eye className="h-4 w-4 sm:h-5 sm:w-5 text-cyan-600 dark:text-cyan-400" />
            </div>
          </div>
          <p className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">
        ...
        <div className="h-40 sm:h-48 flex items-end gap-[2px] sm:gap-1 overflow-x-auto">
          {data.dailyActivity.map((day, idx) => {
            ...
            return (
              <div
                key={idx}
                className="flex-1 min-w-[6px] sm:min-w-[8px] group relative"
              >
                <div
            ...
            <div className="space-y-2 sm:space-y-3">
              {data.topPresentations.map((pres, idx) => (
                <Link
                  ...
                  className="flex items-center gap-2 sm:gap-4 p-2 sm:p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-neutral-800/50 transition group"
                >
                  <div className="flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-slate-100 dark:bg-neutral-800 text-xs sm:text-sm font-bold text-slate-600 dark:text-neutral-300 shrink-0">
                    {idx + 1}
                  </div>
                  ...
                  <ArrowUpRight className="h-4 w-4 text-slate-400 opacity-0 group-hover:opacity-100 transition hidden sm:block" />
                </Link>
              ))}
            </div>
          ) : (
                ...
                return (
                  <div key={type}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs sm:text-sm font-medium text-slate-700 dark:text-neutral-300 capitalize">
                        {type.replace(/_/g, " ")}
                      </span>
      ...
      <div className="bg-gradient-to-br from-[#1e3a8a] to-[#06b6d4] rounded-2xl p-4 sm:p-6 text-white">
