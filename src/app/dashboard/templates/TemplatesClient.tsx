...
export default function TemplatesClient() {
  ...
  const card = (t: TemplateCard, isCurated: boolean) => {
    ...
    return (
      <div
        key={t.id}
        className="group relative flex flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm ring-1 ring-slate-900/5 transition-all duration-300 hover:-translate-y-1 hover:border-teal-400/40 hover:shadow-lg dark:border-white/10 dark:bg-white/[0.04] dark:ring-0 dark:shadow-none dark:hover:bg-white/[0.06]"
      >
        {/* Aurora sheen on hover */}
        <div
          className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100"
          style={{ background: "linear-gradient(135deg, rgba(16,185,129,0.10), transparent 40%, transparent 60%, rgba(45,212,191,0.10))" }}
        />
        <div className="relative flex items-center justify-between gap-2">
          <span className="rounded-full border border-emerald-400/30 bg-emerald-500/10 px-2.5 py-0.5 text-[0.65rem] font-black uppercase tracking-wider text-emerald-600 dark:text-emerald-300">
            {t.category}
          </span>
        ...
        <h3 className="relative mt-3 text-base font-bold text-slate-900 transition-colors group-hover:text-teal-600 dark:text-white dark:group-hover:text-teal-300">
          {t.name}
        </h3>
        {t.description && (
        ...
        <button
          ...
          className="relative mt-4 flex items-center justify-center gap-1.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-500 px-3 py-2 text-xs font-black uppercase tracking-wider text-white transition hover:brightness-110 disabled:opacity-50"
        >
          {usingId === t.id ? (
    ...
    <div className="mx-auto max-w-6xl px-6 py-10">
      <div className="mb-8">
        <h1 className="flex items-center gap-2 text-2xl font-black uppercase tracking-tight text-slate-900 dark:text-white">
          <LayoutTemplate size={20} className="text-teal-500" /> Templates
        </h1>
          ...
          <div className="mb-4 flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className={`rounded-full border px-3 py-1 text-[11px] font-bold transition-colors ${
                activeCategory === null
                  ? "border-transparent bg-gradient-to-r from-emerald-600 to-teal-500 text-white"
                  : "border-slate-200 bg-white text-slate-500 hover:border-teal-400/50 dark:border-white/10 dark:bg-white/5 dark:text-zinc-400"
              }`}
            >
              All {curated.length}
            </button>
